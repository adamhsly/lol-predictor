...
POSITION_SHORT = {"TOP": "top", "JUNGLE": "jg", "MIDDLE": "mid", "BOTTOM": "bot", "UTILITY": "sup"}
...
def extract_draft_features(
    ...
    for pos in POSITION_ORDER:
        ...
        features[f"{short}_mastery_diff"] = bp.get("mastery_points", 0) - rp.get("mastery_points", 0)
        features[f"{short}_wr_diff"] = bp.get("recent_winrate", 0.5) - rp.get("recent_winrate", 0.5)
        features[f"{short}_champ_wr_diff"] = bp.get("champ_winrate", 0.5) - rp.get("champ_winrate", 0.5)
        features[f"{short}_summoner_level_diff"] = bp.get("summoner_level", 0) - rp.get("summoner_level", 0)
        features[f"{short}_wr_residual_diff"] = bp.get("winrate_rank_residual", 0) - rp.get("winrate_rank_residual", 0)
