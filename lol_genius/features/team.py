...
def extract_team_features(
    ...
    ad_count = sum(1 for cf in champion_features if cf.get("is_ap_champ", 0) < 0.5 and cf.get("is_mixed_champ", 0) < 0.5)
    ...
    features["melee_count"] = float(sum(1 for cf in champion_features if cf.get("is_melee", 0) > 0.5))

    features["tank_count"] = float(sum(1 for cf in champion_features if cf.get("tag_tank", 0) > 0.5))
    features["assassin_count"] = float(sum(1 for cf in champion_features if cf.get("tag_assassin", 0) > 0.5))
    features["mage_count"] = float(sum(1 for cf in champion_features if cf.get("tag_mage", 0) > 0.5))
    features["marksman_count"] = float(sum(1 for cf in champion_features if cf.get("tag_marksman", 0) > 0.5))
    features["support_count"] = float(sum(1 for cf in champion_features if cf.get("tag_support", 0) > 0.5))
    ...
    features["hot_streak_count"] = float(sum(1 for pf in player_features if pf.get("hot_streak", 0) > 0.5))
    ...
    features["total_attack_score"] = sum(cf.get("champ_attack_score", 5) for cf in champion_features)
    features["total_defense_score"] = sum(cf.get("champ_defense_score", 5) for cf in champion_features)
    features["total_magic_score"] = sum(cf.get("champ_magic_score", 5) for cf in champion_features)
    features["avg_difficulty"] = sum(cf.get("champ_difficulty", 5) for cf in champion_features) / n_champs

    return features


TEAM_FEATURE_NAMES = [
    "avg_rank", "rank_spread", "avg_team_winrate", "avg_mastery",
    "ad_ratio", "ap_ratio", "melee_count",
    "tank_count", "assassin_count", "mage_count", "marksman_count", "support_count",
    "autofill_count", "damage_diversity",
    "avg_summoner_level", "hot_streak_count", "avg_wards_placed", "avg_cc_score",
    "total_attack_score", "total_defense_score", "total_magic_score", "avg_difficulty",
]
